...
class Handlers
{
    ...
    */
    ...
    static var serverName: String = null;
    static var connectStatus: boolean = true;
    static var logPath: String = null;
    static var errorCounter: int = 0;
    ...
    static function OnBeforeRequest(oSession: Session) {
        ...
        if (oSession.HostnameIs(serverName) && !connectStatus){
		...
		if  (!oSession.url=="api.d.comcash.com/xml/sale") 
		{
...
*/
    ...
    static function GetSessions (arg: String): Boolean {
        var oSessions = FiddlerApplication.UI.GetAllSessions();
        var length = oSessions.Length - 1;
        ...
        if ( thisSession.responseCode != 200){
...
*/
    ...
    case "launch":
                ...
                if (!File.Exists(path))
                {
                    var sc = new StreamWriter(path,true);
                    sc.Close();
                }
                serverName = sParams[2];
                logPath = sParams[3];
                return true;
     case "off":
                connectStatus = false;
     ...
     case "on":
                ...
                return true;
     case "request":
                var res = GetSessions(sParams[1]);
                
                var dw = File.AppendText(path);
                dw.WriteLine(res.ToString());
                dw.Close();
                return true;
